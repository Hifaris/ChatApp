...
import { GET_CONTACT_FOR_DM_ROUTE } from "@/utils/constants";
...
const ContactsContainer = () => {
  ...
  useEffect(()=>{
     const getContact = async()=>{
      const resp = await apiClient.get(GET_CONTACT_FOR_DM_ROUTE,{withCredentials:true})
      console.log(resp)
      if(resp.data.contacts){
        setDirectMessageContacts(resp.data.contacts)
      }
     }
     getContact()
  },[])
...
const Logo = () => {
    return (
      <div className="flex p-5  justify-start items-center gap-2">
        ...
        <span className="text-3xl font-semibold ">FaChat</span>
      </div>
    );
  };
